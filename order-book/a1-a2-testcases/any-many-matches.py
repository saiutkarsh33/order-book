...
def main(num_thr, num_instr, ords_per_instr):
	print(f'''
	# fills up the order book for {num_instr} instruments, then clears it by matching,
	...
	for instr in instr_list:
		for i in range(ords_per_instr // num_thr // 2):
	...
	for instr in instr_list:
		for i in range(ords_per_instr // num_thr // 2):
			...
			for tid in thr_list:
				assert price > 0
				print(f"{tid} B {oid} {instr} {price} {random.randint(250, 690)}")
				price += random.randint(6, 12)
				oid += 1
